  ...
  return (
    ...
    { isDropLine && (
    ...
    }
      {
        isMenuOpen && (
            <>
          <div className="px-4 sm:px-0 fixed isMenu bg-black z-10 inset-0 items-center justify-center text-white">
            <nav className="h-[100px] max-w-[90%] w-full mx-auto flex items-center justify-between">
            <div className="logo"><img src={logo}  className="h-[30px] w-[30px] sm:h-auto sm:w-auto" alt="cregital-logo" /></div>
            <div onClick={toggleMenu} className="flex items-center gap-2 md:gap-4" >
            <img  className='menu' src={cancelMenu} alt="menu-open" />
            <span className="font-ibm thick-text text-[13px] md:text-[15px] text-dark font-bold">CLOSE</span>
            </div>
            </nav>
            <motion.div initial={{y:"400px",opacity:0}} animate={{y:0,opacity:1}} transition={{duration:1}} className="flex mx-auto items-center justify-center hero font-ibm text-[30px] md:text-[70px] font-semibold max-w-[23ch] text-white text-center">
